...
\textbf{\partref{1}{sec:p1}} applies Randomized Hill Climbing (RHC), Simulated Annealing
(SA), and a Genetic Algorithm (GA) to the final two layers
($1{,}159$ trainable parameters, well within the 50k cap);
\textbf{\partref{2}{sec:p2}} runs seven Adam-family optimizer ablations ---
...
\textbf{\partref{3}{sec:p3}} studies five regularizers under fixed best-Part-2 Adam settings:
L2 weight decay, early stopping, dropout, label smoothing, and input noise;
\textbf{\partref{4}{sec:p4}} (extra credit) composes the best pieces from Parts 1--3.
...
highest final Macro-F1 among \partref{2}{sec:p2} variants.
Dropout ($p{=}0.2$) outperforms every \partref{2}{sec:p2} optimizer on Macro-F1; the
...
\textbf{H1 (Optimizer speed, locked before running \partref{2}{sec:p2}).}
...
\textbf{H2 (Regularization vs.\ optimizer, locked before running \partref{3}{sec:p3}).}
...
\textbf{H3 (RO fine-tuning cost, locked before running \partref{1}{sec:p1}).}
...
\secref{sec:data} fixes the Covertype continuity contract;
\secref{sec:p1} through \secref{sec:p4} report Parts~1--4;
\secref{sec:discuss} synthesises across parts;
\secref{sec:conclusion} resolves the hypotheses with quantitative evidence.
...
Balanced Accuracy $0.526$ --- the numbers every \partref{1}{sec:p1}--4 comparison is
measured against.
...
Batch size 256; seeds $\{42, 123, 7\}$ for stability analysis in \partref{2}{sec:p2}.
...
\caption{\partref{1}{sec:p1} --- best-so-far validation loss vs.\ function evaluations
(\emph{left}) and wall-clock time (\emph{right}) for RHC, SA, and GA.
\textcolor{denimdark}{RHC} (solid) descends steeply in the first 200 FEs then plateaus at the local
optimum it cannot escape without restarts; \textcolor{amber}{SA} (\emph{dashed flat}) begins at its best
...
freezing the chain at the initial point; \textcolor{plumcomp}{GA} (\emph{dash-dot}) descends gradually across
...
\begin{table}[t]\centering
\caption{\partref{1}{sec:p1} RO summary vs.\ SL SGD baseline.
...
\caption{\partref{2}{sec:p2} --- validation loss vs.\ GEs (\emph{left}) and wall-clock time
(\emph{right}), all seven variants, seed$=42$.
The \emph{dotted} line marks $\ell_{\text{vis}}{=}0.671$, plotted as $90\%$ of
...
\textcolor{denimdark}{SGD-no-momentum} does not cross it until ${\approx}1{,}300$ GEs, confirming H1.
The \textcolor{sienna}{Adam (no bias-correction)} curve sits above standard Adam for the first
...
{\approx}0.010$) and highest Macro-F1 among \partref{2}{sec:p2} variants, consistent
with decoupled weight decay improving generalization independently of
optimization speed \cite{loshchilov2019adamw}.

\begin{table}[t]\centering
\caption{\partref{2}{sec:p2} optimizer results (seed$=42$, 80 epochs).
...
\figref{fig:stability} plots median~$\pm$~IQR val loss across three seeds per
optimizer.
Adam-family variants cluster in a narrow band throughout training; SGD-no-momentum
has the widest IQR, confirming that adaptive second-moment scaling suppresses the
initialisation-direction sensitivity that makes plain SGD volatile.
This seed stability --- not just convergence speed --- is the argument for
preferring Adam-family methods in production settings where a single training
run determines the deployed model.

% =============================================================================
\section{Part 3: Regularization Study}\label{sec:p3}

\subsection{Design}\label{sec:p3:design}
All \partref{3}{sec:p3} experiments use standard Adam with the best \partref{2}{sec:p2} hyperparameters
(lr$=10^{-3}$, $\beta_1{=}0.9$, $\beta_2{=}0.999$, $\varepsilon{=}10^{-8}$,
no weight decay) and the same 80-epoch budget.
Adam is not re-tuned in \partref{3}{sec:p3} --- any change in outcomes is attributable
...
\caption{\partref{3}{sec:p3} --- validation-loss trajectories (\emph{left}, seed=42) and
$\Delta$Macro-F1 vs.\ Adam baseline (\emph{right}, median $\pm$ IQR, 3 seeds;
sorted worst-to-best; green zone $=$ gain, sienna zone $=$ loss).
Only three regularizers improve over the baseline: L2 ($\lambda{=}10^{-4}$,
$+0.028$), input noise ($+0.023$), and early stopping ($+0.015$).
Dropout and the best-combo both hurt Macro-F1, indicating over-regularization
of the majority-class signal.
Label smoothing is near-neutral ($-0.007$): it lowers reported validation loss
by softening CE targets without shifting minority-class decision boundaries
--- calibration and discrete-class decisions are separable outcomes
\cite{muller2019labelsmoothing}.
The delta framing makes the $\pm 0.08$ range invisible in an absolute
bar chart immediately legible.}
...
evaluation run (\figref{fig:reg}), with reduced train--validation gap and
stable behaviour across seeds (IQR$<0.003$).
...
The train--validation loss gap at budget end (\figref{fig:gap}) shows
early stopping and dropout narrowing the gap most effectively, while
label smoothing inflates the apparent gap because CE is computed on
softened targets during training but hard targets during validation.
...
H2 is supported: the best \partref{3}{sec:p3} combination outperforms the best \partref{2}{sec:p2}
...
\partref{4}{sec:p4} composes the best pieces from Parts 1--3 under the constraints
the spec imposes:
\begin{itemize}[leftmargin=*]
\item \textbf{Optimizer}: standard Adam with \partref{2}{sec:p2} best hyperparameters.
\item \textbf{Regularization}: best \partref{3}{sec:p3} combination
(L2 $10^{-4}$ + early stopping patience-10 + label smoothing $\varepsilon{=}0.1$).
\item \textbf{RO fine-tuning}: best \partref{1}{sec:p1} algorithm (RHC) on the same
final two layers, $\leq10\%$ of the \partref{1}{sec:p1} budget
...
\begin{table}[t]\centering
\caption{\partref{4}{sec:p4} integrated recipe vs.\ all prior bests.
...
\midrule
SL SGD baseline       & $0.538$ & $0.590$ & $0.526$ & --- \\
Best RO (RHC, \partref{1}{sec:p1}) & $0.563$ & $0.594$ & $0.530$ & 1{,}203 FE \\
Best optimizer (AdamW) & $0.495$ & $0.671$ & $0.559$ & 3{,}760 GE \\
Best regularization    & $0.511$ & $0.653$ & $0.588$ & 3{,}760 GE \\
\textbf{Integrated (\partref{4}{sec:p4})} & $\mathbf{0.483}$ & $\mathbf{0.672}$ &
...
(\tabref{tab:p4}) --- marginally above AdamW alone ($+0.001$ Macro-F1) and
above the best regularization combination ($+0.019$ Macro-F1).
...
\textbf{optimizer speed} (\partref{2}{sec:p2}) and \textbf{generalization} (\partref{3}{sec:p3}) are
...
The Macro-F1 spread across all seven \partref{2}{sec:p2} optimizers (range $0.021$) is
smaller than the \partref{3}{sec:p3} regularization spread (range $0.033$).
...
After SL pre-training fixes the backbone on those axes, \partref{2}{sec:p2}'s optimizer
...
The $0.021$ Macro-F1 range across all seven \partref{2}{sec:p2} optimizers is
...
\partref{3}{sec:p3} regularization \emph{does} move the floor because it directly
...
\textit{Generalization (Macro-F1)}: best-combo (\partref{3}{sec:p3}) $>$ AdamW $\approx$
...
\textit{Compute efficiency (Macro-F1 per GE)}: \partref{3}{sec:p3} combinations deliver
...
(\partref{2}{sec:p2} range $0.021$), while the \emph{regulariser} is a strong one
(\partref{3}{sec:p3} range $0.033$).
...
The result of \partref{1}{sec:p1} is a concrete NFL illustration --- RHC, SA, and GA
...
\textbf{H2 --- Supported.}
The best \partref{3}{sec:p3} combination (L2 $+$ early stopping $+$ label smoothing,
\tabref{tab:p4} and \figref{fig:reg})
achieves Macro-F1 $0.653$, outperforming the best \partref{2}{sec:p2} optimizer
...
any \partref{2}{sec:p2} or \partref{3}{sec:p3} experiments; every reported number was re-produced by the
